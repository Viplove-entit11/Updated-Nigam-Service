...
const ConfirmRequest = () => {
  const { setConfirmRequestList } = useAuth();
  ...
  return (
    <div className="total-request-div">
          ...
          {confirmRequestList.map((confirmRequest, index) => (
            <tr key={index}>
              ...
              <td style={{ textAlign: "center" }}>
                {confirmRequest.status === 2 && <p className="status">Completed</p>}
              </td>
      ...
      {/* Pagination Controls */}
      <div className="pagination" style={{ display: "flex", justifyContent: "end", alignItems:"center", gap:"10px"}}>
        <button
